...
# === Ersatz für ehemalige global_state.py ===
last_feed_time_global = 0.0
last_candle_ts_global = 0.0

# === Ersatz für gui_bridge.py: settings werden direkt verwendet ===
# update_status(msg) → print(msg)

# === from data_provider.py – Funktionen direkt eingefügt ===
# data_provider.py
    ...
    def _on_message(self, ws: Any, message: str) -> None:
        try:
            ...
            if now - candle_ts > 90:
                return
#             import global_state
            if last_candle_ts_global is not None and candle_ts <= last_candle_ts_global:
                return
            candle = {
                "timestamp": candle_ts,
                "open": float(k.get("o")),
            ...
            last_feed_time_global = time.time()
            if self.on_candle:
                self.on_candle(candle)
                last_candle_ts_global = candle_ts
        except Exception:
...
#     from andac_entry_master import get_filter_config
        ...
        if not position:
            if entry_type:
                ...
                if sl_tp_mode == "manual":
                    sl = settings.get('manual_sl', 0.75)
                    tp = settings.get('manual_tp', 1.5)
                else:
                    try:
    ...
    while capital > 0 and not getattr(app, "force_exit", False):
        ...
        if not getattr(app, "feed_ok", True):
            print(
                f"🧪 Letzter Feed-Eingang vor {time.time() - last_feed_time_global:.1f} Sekunden"
            )
            time.sleep(1)
            continue
...
def _run_bot_live_inner(settings=None, app=None):
    ...
    capital = float(settings.get('capital', 2000)) if gui_bridge else settings.get(
        "capital", 2000
    )
    ...
    multiplier = settings.get('multiplier', 10)
    capital = float(settings.get('capital', 2000))
    start_capital = capital
    interval = interval_setting
    auto_multi = settings.get('auto_multiplier', False)
    leverage = multiplier
    live_requested = settings.get('paper_mode') is False
